...
if __name__ == '__main__':
    ...
    for epoch_idx in range(init_epoch, n_epoch):
        ...
        for batch_idx, (x, l) in enumerate(train_loader):
            ...
            gmvae_loss = criterion.gmvae_m2_loss(params, largs, reduction='none')
        ...
        if epoch % eval_itvl == 0:
            with torch.no_grad():
                ...
                time_start = time.time()
                z_xy = torch.Tensor().to(device)
                z_y = torch.Tensor().to(device)
                labels_true = []
                labels_pred = []
                ...
                for batch_idx, (x, l) in enumerate(train_loader):
                    x = x.to(device)
                    params = model(x, return_params=True)
                    z_xy = torch.cat((z_xy, params['z_xy']), 0)
                    z_y = torch.cat((z_y, params['z_y']), 0)
                ...
                with mp.Pool(6) as pool:
                    z_xy_tsne = pool.apply_async(
                        tsne.fit_transform, (z_xy, )).get()
                    z_y_tsne = pool.apply_async(
                        tsne.fit_transform, (z_y, )).get()
                ...
                # output plots
                ut.scatter(z_xy_tsne[:, 0], z_xy_tsne[:, 1],
                           labels_true, f'{outdir}/zxy_tsne_{epoch}_t.png')
                ut.scatter(z_xy_tsne[:, 0], z_xy_tsne[:, 1],
                           labels_pred, f'{outdir}/zxy_tsne_{epoch}_p.png')
                ut.scatter(z_y_tsne[:, 0], z_y_tsne[:, 1],
                           labels_true, f'{outdir}/zy_tsne_{epoch}_t.png')
                ut.scatter(z_y_tsne[:, 0], z_y_tsne[:, 1],
                           labels_pred, f'{outdir}/zy_tsne_{epoch}_p.png')
